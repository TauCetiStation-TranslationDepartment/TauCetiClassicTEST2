...
/datum/tickets/proc/stat_entry(client/target)
	...
	if(check_rights_for(target, R_ADMIN|R_SERVER))
...
/datum/tickets/proc/ID2Ticket(id)
	if(!check_rights((R_ADMIN|R_SERVER|R_MENTOR), TRUE))
...
/datum/ticket/proc/Action(action)
	...
	switch(level)
		if(0)
			if(!check_rights_for(usr.client, (R_ADMIN|R_SERVER|R_MENTOR)))
				return
		if(1)
			if(!check_rights_for(usr.client, (R_ADMIN|R_SERVER)))
				return
