...
/datum/tickets/tgui_data(mob/user)
	...
	if(user.client.selected_ticket)
		...
		if(check_rights_for(user.client, (R_ADMIN|R_SERVER)) || (check_rights_for(user.client, R_MENTOR) && T.level < 1))
			selected_ticket = list(
				...
				"actions" = check_rights_for(user.client, (R_ADMIN|R_SERVER)) ? T.FullMonty(,TRUE) : T.FullMonty(),
				"log" = T._interactions,
			)
	...
	data["is_admin"] = check_rights_for(user.client, (R_ADMIN|R_SERVER))
...
/datum/ticket/tgui_data(mob/user)
	...
	data["actions"] = check_rights_for(user.client, (R_ADMIN|R_SERVER)) ? FullMonty(ref_src, TRUE) : FullMonty(ref_src)
...
/datum/ticket/proc/TicketPanelLegacy(mob/user)
	...
	dat += "<br><br>"
	if(initiator)
		dat += span_bold("Actions:") + " [check_rights_for(user.client, (R_ADMIN|R_SERVER)) ? FullMonty(ref_src, TRUE) : FullMonty(ref_src)]<br>"
	else
...
/datum/tickets/proc/TicketListLegacy(mob/user, state)
	...
	if(state == "Active")
		for(var/datum/ticket/T as anything in GLOB.tickets.active_tickets)
			if(!check_rights_for(user.client, (R_ADMIN|R_SERVER)) && T.level > 0)
				continue
